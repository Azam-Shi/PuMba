...
# Reference: https://github.com/stebliankin/piston
"""
...
class PDB_complex_training(Dataset):
                ...
                with open(iRMSD_file, 'r') as f:
                    ...
                    for line in f.readlines():
                        ...
                        if not os.path.exists(
                                self.data_prepare_dir + '/07-grid/' + model_ppi_i + '.npy'):  # skip if grid doesn't exist
                            continue
    ...
    def __getitem__(self, i):
        ...
        pos_paths = [f"{self.data_prepare_dir}/07-grid/{x}.npy" for x in pos_complexes]
        neg_paths = [f"{self.data_prepare_dir}/07-grid/{x}.npy" for x in neg_complexes]
        ...
        energy_dir_docked = f"{self.data_prepare_dir}/07-grid/"
        ...
        pos_labels = [1] * len(pos_grids)
        neg_labels = [0] * len(neg_grids)
        
        # Combine and shuffle
        combined = list(zip(pos_grids + neg_grids, pos_energies + neg_energies, pos_labels + neg_labels))
        random.shuffle(combined)
        
        # Unpack
        grid, all_energies, labels = zip(*combined)
        grid = np.stack(grid).astype(np.float32)
        grid = np.swapaxes(grid, -1, 1)
