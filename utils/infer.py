...
import torch
import numpy as np
from networks.Pumba_Net import PuMba_proto
from networks.ViM_hybrid import get_ml_config
from utils.dataset import PuMba_dataset
...
def construct_default_config(pdb_dir, out_dir):
    ...
    config['model'] = os.path.dirname(os.path.abspath(__file__)) + "/../saved_models/pumba.pth"
...
def infer_from_model(ppi_list, grid_dir, model_path, params, device, radius, vis_dir):
    ...
    model = PuMba_proto(model_config, img_size=radius * 2,
                             num_classes=2).float().to(device)

    model.load_state_dict(torch.load(model_path, map_location=device))
    model_parameters = filter(lambda p: p.requires_grad, model.parameters())
    n_params = sum([np.prod(p.size()) for p in model_parameters])
    print(f"Loaded PuMba model with {n_params} trainable parameters. Radius of the patch: {radius}A")

    ## Constructing a dataset
    dataset = PuMba_dataset(grid_dir, ppi_list)
...
def infer_cmd(args):
    """
    Obtain PuMba scores (from command line)
    """
    ...
    with open(out_dir+'/PuMba_scores.csv', 'w') as out:
        out.write("PPI,PuMba_score\n")
        for i,ppi in enumerate(ppi_list):
            out.write(f"{ppi},{scores[i]}\n")
    print(f"PuMba is complete. See output in {out_dir}")
